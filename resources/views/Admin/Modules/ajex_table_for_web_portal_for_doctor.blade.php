...
<?php
    ...
    {
        if($table_column == 'hide__or__show')
        {
            continue;
        }
...
<div class="d-flex mb-3" style="justify-content: space-between; width: 37%;">
        ...
        <button type="button" class="btn btn-primary btn-sm" onclick="fetch_all_data('{{$module_id}}','{{$table_name}}','0',[{key:$('#fillter_key').val(),value:$('#fillter_value').val()}],getCookie('section_patient'));">search</button>
        <button type="button" class="btn btn-primary btn-sm" onclick="fetch_all_data('{{$module_id}}','{{$table_name}}','0','',getCookie('section_patient'));">x</button>
        </div>
    <table id="uploadCsvModalTable" class="display">
        ...
        for($i = 0; $i < count($table_data);$i++)
            {
                  ...
                  for($j = 0; $j<count($table_columns);$j++)
                  {
                    if($table_columns[$j] == "hide__or__show")
                    {
                        ...
                        {
                            $edit_id_field .='<td class="d-none" ><i role="button" onclick="hide_or_show('.$table_data[$i]["id"].',\''.$table_name.'\',0);"  style="font-size:20px;color:#02b2b0;" class="ms-4 text-center fa-sharp fa-solid fa-eye-slash" ></i></td>';

                        }
                        else
                        {
                            $edit_id_field .='<td class="d-none" ><i role="button" onclick="hide_or_show('.$table_data[$i]["id"].',\''.$table_name.'\',1);"  style="font-size:20px;color:#02b2b0;" class="ms-4 text-center fa fa-eye" ></i></td>';
        ...
        <a role="button" onclick="fetch_all_data('{{$module_id}}','{{$table_name}}','{{($i-1)*10}}',[{key:$('#fillter_key').val(),value:$('#fillter_value').val()}],getCookie('section_patient'));" class="p-btn<?php if( ($from+1) >= (($i-1)*10+1) && ($from+1) <= ((($i-1)*10)+10) ){ echo  ' active'; } ?>"   >{{$i}}</a>
        @endfor
