...
<div class="page-body">
   <div class="container-xl">
      <div class="row row-cards">
         ...
         <form method="POST" class="page_form" action="{{route('Admin.general_settings.pages.update')}}"  enctype="multipart/form-data">
            @csrf
         <div class="row ">
            ...
            <div class="form-group col-6">
               <div class="bg-dark" style="width:50px;height:50px;margin-top: 20px;"><img src="/" class="logo_pic" style="width:50px;height:50px;" ></div>
            </div>
...
$(".page_form").on("submit", function(e) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{{route('Admin.general_settings.pages.update')}}",
...
$.ajax({
    ...
    type: "get",
    url: "{{url('api/pages/'.request()->route('id'))}}",
    success: function(response) {
        loader(false);
        tinymce.get("description").setContent(response.data.page.description);
        $('.role').val(response.data.page.role_id);
        $('.name').val(response.data.page.name)
        $('.logo_pic').attr('src',response.data.page.logo)
    },
    error: function(response) {
        loader(false);
        if (response.status == 422) {
